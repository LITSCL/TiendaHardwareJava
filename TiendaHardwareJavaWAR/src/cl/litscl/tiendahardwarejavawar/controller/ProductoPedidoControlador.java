package cl.litscl.tiendahardwarejavawar.controller;

import java.io.IOException;

import javax.inject.Inject;
import javax.servlet.ServletException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;

import cl.litscl.tiendahardwareejb.bean.ProductoPedidoDAOLocal;
import cl.litscl.tiendahardwareejb.model.ProductoPedido;

/**
 * Servlet implementation class ProductoPedidoControlador
 */
@WebServlet("/ProductoPedido")
public class ProductoPedidoControlador extends HttpServlet {
	private static final long serialVersionUID = 1L;
	@Inject
	private ProductoPedidoDAOLocal daoProductoPedido;
	
	private ProductoPedido pp = new ProductoPedido();
	
	private int id;
	private int unidades;
	private int productoFK;
	private int pedidoFK;
	
	private String jpql;
	
	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public ProductoPedidoControlador() {
	    super();
	    // TODO Auto-generated constructor stub
	}
	
	/**
	 * @see HttpServlet#doGet(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// TODO Auto-generated method stub
		response.getWriter().append("Served at: ").append(request.getContextPath());
	}

	/**
	 * @see HttpServlet#doPost(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// TODO Auto-generated method stub
		doGet(request, response);
	}

}